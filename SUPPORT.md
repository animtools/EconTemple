# サポート・フィードバック

EconTemple に関する不具合報告・ご質問・「使ってみた」の感想をお待ちしています。

---

## 連絡先


| 内容           | 窓口                                                                              |
| ------------ | ------------------------------------------------------------------------------- |
| **不具合報告**    | [GitHub Issues](https://github.com/animtools/EconTemple/issues)                 |
| **質問・要望・感想** | 上記 Issues または **[shinosuke.spark@gmail.com](mailto:shinosuke.spark@gmail.com)** |


---

## 不具合報告の際に教えてください

- **現象**: 何が起きたか
- **再現手順**: できる範囲で
- **環境**: Windows のバージョン（10/11、64bit）
- **ファイル**: 問題が特定のコンテ・PDF で発生する場合、形式やサイズの目安

---

## 関連リンク

- [QuickStart.md](./QuickStart.md) — 操作手順（画面キャプチャ付き）
- [SAMPLE_WALKTHROUGH.md](./SAMPLE_WALKTHROUGH.md) — 同梱サンプルでの体験手順

画面ごとの詳しい操作説明書やよくある質問の一覧は現在公開していません。ご質問・不具合・要望は上記の連絡先へお寄せください。